...
// keep track of if the window.load event has fired as impossible to check after the fact
    ...
    Plugin.prototype = {
        ...
        _init: function () {
            ...
            //在输入框前面添加地区码
            if (this.options.appendNationCode) {
                var that = this;
                that.telInput.before('<div class="intl-code-detail">+' + that.getSelectedCountryData().dialCode + '</div>').on("keyup change", function () {
                    that.telInput.prev(".intl-code-detail").text("+" + that.getSelectedCountryData().dialCode).end()
                        //自动设置input的位置
                        .css("padding-left", that.telInput.prev(".intl-code-detail").width() + 50);
                }).css("padding-left", that.telInput.prev(".intl-code-detail").width() + 50);
            }
